class Pagination {
  ...
  init(paramsObj) {
    ...
    // 当需要省略符号占位时，确定active的位置,方便后面确定左省略和右省略的临界值
    //左边省略的临界值：maxShowBtnCount 右边： state.totalPageCount-(state.maxShowBtnCount - state.activePosition) -1
    state.totalPageCount > state.maxShowBtnCount + 2 &&
      (state.activePosition = Math.ceil(state.maxShowBtnCount / 2))
    this.renderPageDOM()
  }

  //这个方法主要用来获取pageNumber的值
  switchPage() {
    let state = this.state
  ...
  gotoPage(pageNumber) {
    ...
    if (state.activePosition) {
      ...
      if (pageNumber > state.maxShowBtnCount && pageNumber < rEllipseSign) {
        ...
        for (let i = 1; i < state.maxShowBtnCount + 1; i++) {
          ...
          evaNumberLi[i].setAttribute(
            ...
            pageNumber + (i - state.activePosition)
          )
        }
      ...
      if (pageNumber >= rEllipseSign) {
        ...
        if (
          +evaNumberLi[len - 2].getAttribute(state.dataNumberAttr) <
  ...
  renderPageDOM() {
    ...
    if (totalPageCount - 2 > maxShowBtnCount) {
      ...
      for (let i = 2; i < maxShowBtnCount + 2; i++) {
        paginationStr += `<li class="${pCName} ${pageNumberCName} ${
          i === 1 ? activeCName : ""
        }" ${dataNumberAttr}='${i}'>${i}</li>`
      }
      paginationStr += `
    ...
    paginationStr += `<li class="${pCName} ${nextCName}${
